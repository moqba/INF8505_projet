...
public:
    ...
    pdc_commands(int coreid, string server, int port=41001);
    ~pdc_commands();
 ...
 // hardware breakpoints/watchpoints :
    int watch_supported(const Watchpoint_settings*, bool& aligned_pairs);
 ...
 // blocked based read access of storages :
    ...
    DataType received_next_mem(string)         { return received_next(); }
    DataType received_next_reg(string);
    unsigned address_step(string)               { return 1; /* TODO */}
 ...
 // vectorised access to registers and memories :
    unsigned vector_size(string name) { return 0; }//name == "PMb" ? 4 : 0; }
    int get_register(string name, pdc_caches::range elems);
    int get_memory_at(string name, AddressType address, pdc_caches::range elems);
    ...
    int put_memory_at(string name, AddressType address, pdc_caches::range elems,
                      const std::vector<DataType>& vals);
...
private:
 ...
 // breakpoint instruction codes: 001bbbii
    ...
    std::map<string,string>  bp_root_mem;
    std::map<string,std::array<int,nr_breakpoints> > bp_watch_load_instr;
    std::map<string,std::array<int,nr_breakpoints> > bp_watch_store_instr;
 ...
 // processor move instructions to access registers
    std::map<string,instr_type> reg_get_code;
    std::map<string,instr_type> reg_put_code;

    instr_type goto_r1_code;
 ...
 // current value of ocd_address_register, incremented on each st/ld :
    ...
    void put_address_register(AddressType a) {
...
inline int pdc_commands::watch_supported(const Watchpoint_settings* ws, bool& aligned_pairs)
{
                                          ...
                                          const Watchpoint_settings* ws)
{
    ...
    if (ws->block_size > 1) {
        ...
        if (verb) {
            cerr << "PDC enable range watchpoint " << nrs.front() << "/" << nrs.back()
...
inline AddressType pdc_commands::get_program_counter()
{
...
{
    DataType v(32,false); v = 0;
    for (int i = 0; i < 4; i++)   // LITTLE endian
        v.deposit(received_next(), (i+1)*8-1, (i)*8);
    return v;
}
...
inline int pdc_commands::get_break_context()
{
    if (interleaved_contexts) {
        ...
        int val = ocd_current_context;  // restore ocd_context_register
        cbl.add_dwrite_dread(ocd_cable::ctxt,val);
        send_and_receive();
        val = received_next().to_int();
