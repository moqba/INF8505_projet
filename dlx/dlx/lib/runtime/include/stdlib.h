...
#endif
inline void clib_exit(int status) property(never_returns)
{
...
inline void exit(int status) property(never_returns) {
    __cxa_finalize(NULL);
    clib_exit(status);
}

inline void abort() property(never_returns) { clib_exit(127); }

inline void _Exit(int status) property(never_returns) { clib_exit(status); }
