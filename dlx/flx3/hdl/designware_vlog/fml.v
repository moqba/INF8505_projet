...
module fml
  ...
  reg    signed [data_width-1:0] fmlA;
  reg    signed [data_width-1:0] fmlB;
  wire   signed [data_width-1:0] fmlC;
  wire   signed [ 2:0] fmlRND = 3'b000; // round to nearest (even)
  wire   signed [ 7:0] fmlSTAT;  // unused

  reg    signed [data_width-1:0] fmlC_DLY1;
  ...
  begin : p_fml_isolate
    ...
    begin
      fmlA = fmlA_in[data_width-1:0];
      fmlB = fmlB_in[data_width-1:0];
    end
  end
...
`ifdef VERILATOR_COMBINATORIAL_VIEW
  assign fmlC_out = { 8'b0, fmlC };
`else // Pipelined implementation view
  always @ (posedge clock)
  begin : p_ms_fml
    fmlC_DLY1 <= fmlC;
  end
  assign fmlC_out = { 8'b0, fmlC_DLY1 };
`endif
