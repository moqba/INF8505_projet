...
//#define     PRINT_DIV
#ifdef        PRINT_DIV
#define DEBUG_PRINT_DIV(FMT,ARGS...) DEBUG_PRINT("div: "FMT, ##ARGS)
#else
#define DEBUG_PRINT_DIV(FMT,ARGS...)
#endif
...
multicycle_fu div
{
    ...
    process main () {
        ...
        // steps
        if (cnt > 1) {
            DEBUG_PRINT_DIV("step %x",PA);
            PA = div_step(PA, B);
            new_cnt = cnt - 1;
        }
        // post-amble
        if (cnt == 1) {
            DEBUG_PRINT_DIV("post-amble");
            uint64_t pa = PA;
        ...
        cnt = new_cnt;
        div_busy_raw = ((cnt > 0) || (new_cnt > 0)) && (context_id_ID == C);
        bool l_div_busy_wb = (cnt > 3) && (context_id_ID == C);
        if (cnt==0) l_div_busy_wb |= (new_cnt > 0) && (context_id_ID == context_id_EX);
        div_busy_wb = l_div_busy_wb;
