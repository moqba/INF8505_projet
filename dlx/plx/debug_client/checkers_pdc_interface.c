...
Checkers_pdc_interface::Checkers_pdc_interface(int coreid, string server, int port)
    : Checkers_debugger(coreid), cmd(coreid,server,port)
{
...
void Checkers_pdc_interface::reset_pdc()
{
 ...
 // remove software breakpoints
    map<unsigned,pmem_instr_pair>::iterator p;
...
pdc_commands::instr_type Checkers_pdc_interface::get_instruction_at(unsigned pc)
{
    long long instr;
...
void Checkers_pdc_interface::put_instruction_at(unsigned pc,
                                                pdc_commands::instr_type instr)
{
    put_memory_at(cmd.pmem_name,pc,instr);
}

int Checkers_pdc_interface::add_sw_break(unsigned pc, bool export_)
{
...
int Checkers_pdc_interface::remove_sw_break(unsigned pc)
{
...
{
    unsigned pc = get_program_counter();   // force status check
...
unsigned Checkers_pdc_interface::get_program_counter()
{
    if (pdc_commands::sw_break_enabled && status != stopped) {
        Core_status cs;
        get_status(&cs);  // force status check before returning PC
    }
    map<string,long long>::const_iterator p = reg_cache.find(cmd.pc_name);
    if (p != reg_cache.end())
        return (unsigned)p->second;
    else
        return reg_cache[cmd.pc_name] = cmd.get_program_counter();
}

int Checkers_pdc_interface::get_register(string nm, long long& val)
{
    to_lower(nm);
    if (nm == cmd.pc_name) {
    ...
    map<string,long long>::const_iterator p = reg_cache.find(nm);
    if (p != reg_cache.end())
        val = p->second;
...
int Checkers_pdc_interface::put_program_counter(unsigned val)
{
    if (pdc_commands::sw_break_enabled && status != stopped) {
        Core_status cs;
...
int Checkers_pdc_interface::put_register(string nm, long long val)
{
    to_lower(nm);
    if (nm == cmd.pc_name)
        return put_program_counter((unsigned)val);
    reg_cache.clear();
    reg_cache_loaded = false;
...
void Checkers_pdc_interface::protect_memory_at(string nm,
                                               unsigned first, unsigned last)
{
    mem_protections[nm].push_back(std::make_pair(range(first,last),false));
...
typedef std::pair<pdc_caches::range,bool> pair_r_b;
...
inline bool overlaps(vector<pair_r_b>& v, range r)
{
    for (vector<pair_r_b>::iterator i = v.begin(); i < v.end(); ++i)
...
inline
vector<pair_r_b>* overlaps(map<string,vector<pair_r_b> >& m, string nm, range r)
{
...
int Checkers_pdc_interface::get_memory_at(string nm, unsigned addr,
                                          long long& val)
{
 ...
 // cache-based access while excluding protected addresses
    map<unsigned,cache_line>& cache = mem_caches[nm];
    unsigned indx = pdc_caches::cache_index(addr);
    unsigned offs = pdc_caches::cache_offset(addr);
    map<unsigned,cache_line>::iterator p = cache.find(indx);
    if (p != cache.end())
        val = p->second[offs];
    else {
     // load cache line
        cache_line& line = cache[indx];
        range r = pdc_caches::line_address_range(addr);
        ...
        for (unsigned i = r.first; i <= r.last; i++)
            if (protect && overlaps(*protect,i))
                ...
                line[i-r.first] = cmd.received_next_mem(nm);
        val = line[offs];
    }
    return ok;
}
...
int Checkers_pdc_interface::put_memory_at(string nm, unsigned addr,
                                          long long val)
{
    unsigned vec_sz = cmd.vector_size(nm);
    if (vec_sz > 1) {
    ...
    mem_caches[nm].erase(pdc_caches::cache_index(addr));
                                          ...
                                          unsigned elem, long long& val)
{
    ...
    map<unsigned,vcache_line>& cache = mem_vcaches[nm];
    unsigned indx = pdc_caches::vcache_index(addr,elem,vec_sz);
    unsigned offs = pdc_caches::vcache_offset(addr,elem);
    map<unsigned,vcache_line>::iterator p = cache.find(indx);
    if (p != cache.end())
        val = p->second[offs];
    else {
        vcache_line& cache_line = cache[indx];
        range ar = pdc_caches::vline_address_range(addr);
        ...
        if (merge_line.covered(ar,er))
            merge_line.flush(this,&cmd,nm);  // flush pending writes
        vector<pair_r_b>* protect = overlaps(mem_protections,nm,ar);
        for (unsigned a = ar.first; a <= ar.last; a++)
            if (!(protect && overlaps(*protect,a))
                && cmd.get_memory_at(nm,a,er) != ok) return failed;
        cmd.send_and_receive();
        vector<long long> vals(er.length());
        long long* p = cache_line.begin();
        for (unsigned a = ar.first; a <= ar.last; a++) {
            if (protect && overlaps(*protect,a))
...
int Checkers_pdc_interface::get_protect_memory_at(string nm, unsigned addr,
                                                  unsigned elem, long long& val)
{
    unsigned vec_sz = cmd.vector_size(nm);
    range er = pdc_caches::vline_elem_range(elem,vec_sz);
    ...
    if (vline.empty()) {
        if (read_once.same_line(nm,addr,r))
         ...
         // clear corresponding cache entry
            ...
            map<unsigned,vcache_line>::iterator p = cache.find(indx);
            if (p != cache.end()) {
                vline.assign(r,&p->second[pdc_caches::vcache_offset(addr,0)]);
                cache.erase(p);
            }
        }
    }
...
void write_merge_vline::flush(Checkers_pdc_interface* pdc,
                              pdc_commands* cmd, string nm)
{
    vector<long long> vals(elems.length());
    if (!complete()) {
        // read missing values
        if (for_reg)
...
void Checkers_pdc_interface::flush_write_merge_buffers()
{
    ...
    for (MI p=write_merge_buffers.begin(); p != write_merge_buffers.end(); ++p)
        if (!p->second.empty())
            p->second.flush(this,&cmd,p->first);
}
...
int Checkers_pdc_interface::load_reg_vcache(const vector<string>& regs, range r)
{
    ...
    for (VI p = regs.begin(); p != regs.end(); ++p) {
        if (write_merge_buffers.count(*p)) {
            ...
            if (merge_line.same_line(r))
                merge_line.flush(this,&cmd,*p);  // flush pending writes
            write_merge_buffers.erase(*p);
        }
    }
    for (VI p = regs.begin(); p != regs.end(); ++p)
    ...
    vector<long long> vals(r.length());
    for (VI p = regs.begin(); p != regs.end(); ++p) {
