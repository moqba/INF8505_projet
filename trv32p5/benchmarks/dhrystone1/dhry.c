...
#endif
#ifdef CHESS_CYCLE_COUNT
  Begin_Time = chess_cycle_count();
  chess_profile_begin();
#endif
...
#endif
#ifdef CHESS_CYCLE_COUNT
    // User_Time corresponds to clock cycles, assume 1MHz clock
    Microseconds = ((float) User_Time * Mic_secs_Per_Second ) / 1.0E6;
    Microseconds = Microseconds / Number_Of_Runs;
    Dhrystones_Per_Second = 1000000 / (float) Microseconds;
#endif
    DMIPS_Per_MHz = Dhrystones_Per_Second / 1757;
