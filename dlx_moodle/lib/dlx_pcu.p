...
#include "isg/dlx_pdg_encoding.p"
...
bool check_interrupts(uint16_t  i_req,     // interrupt requests
                      ...
                      uint7_t& i_vector,   // interrupt vector
...
void dlx::user_issue()
{
   ...
   // interrupts
    ...
    if (trn_interrupted) {
        ...
        fi3 = instr[7:0];
        issue_instr(0, 0, fi0, fi1, fi2, fi3);
        // because we have four bits in PM, but second argument in issue_instr
        // is zero, it is valid bit, so it will not issue
    }
...
#ifdef ENABLE_HAZARD_COVERAGE
#include "coverage_hazards.cov"
#endif
